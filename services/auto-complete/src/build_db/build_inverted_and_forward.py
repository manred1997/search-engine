...
def main():
    ...
    with open('completions' + ".dict") as f:
        for line in f:
            t = line.rstrip('\n')
            tokens[t] = id
            id += 1
    ...
    lines = 0
    inverted = open('completions' + ".inverted", 'w') # inverted lists
    forward = open('completions' + ".forward", 'w') # sets

    num_terms = 0
    num_docs = 0
    with open('completions' + ".mapped.stats") as f:
