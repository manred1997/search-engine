...
from src.helpers.synthetic import prefixSyntheszie
from src.utils.normalize import normalize_diacritic, normalize_encode
...
def process(
    train_start="2006-03-01 00:00:00",
    ...
    # normalize time
    itv = {
        k: tuple(datetime.datetime.strptime(x, fmt) for x in v) for k, v in itv.items()
    }
    ...
    f = {
        s: {
            column: open(os.path.join(target_dir, f"{s}", f"{column}.txt"), "w")
    ...
    for i in range(1, 11):
        ...
        for line in tqdm(f_org):
            data = {
                column: v for column, v in zip(columns, line.strip().split("\t")[:3])
            }
            # normalize queries
            ...
            if (
                len(data["query"]) < 3
                or len(data["query"].split()) < 2
                or (data["uid"], data["query"]) == (prev["uid"], prev["query"])
            ):
                continue
