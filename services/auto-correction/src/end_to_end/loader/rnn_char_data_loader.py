...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for char in input_sentence:
            input_ids.append(tokenizer.convert_char_to_id(char) if char in tokenizer.char_to_id \
        ...
        for char in label_sentence:
            label_ids.append(tokenizer.convert_char_to_id(char) if char in tokenizer.char_to_id \
                            else tokenizer.convert_char_to_id(unk_char))
...
def load_and_cache_examples(
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features(
            examples=examples,
            max_seq_tokens=args.max_seq_len,
            tokenizer=tokenizer
        )
    ...
    all_input_ids = pad_sequence(all_input_ids, batch_first=True, padding_value=tokenizer.convert_char_to_id(tokenizer.pad_char))
    all_attention_mask = [torch.tensor(f.attention_mask) for f in features]
    all_attention_mask = pad_sequence(all_attention_mask, batch_first=True, padding_value=pad_token_label_id)
    all_input_sentences = [f.input_sentence for f in features]

    all_label_ids = [torch.tensor(f.label_ids) for f in features]
    all_label_ids = pad_sequence(all_label_ids, batch_first=True, padding_value=tokenizer.convert_char_to_id(tokenizer.pad_char))
    ...
    assert len(all_input_ids) == len(all_input_sentences) == len(all_label_ids) == len(all_label_length) == len(all_label_sentences)
