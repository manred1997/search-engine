...
#                 )
            ...
            if punc_preds is None:
                ...
                out_punc_labels = inputs["labels"].detach().cpu().numpy()
            else:
                if self.args.use_crf:
                ...
                out_punc_labels = np.append(
                    out_punc_labels, inputs["labels"].detach().cpu().numpy(), axis=0
                )
            
            if label_masks is None:
                label_masks = l_mask.detach().cpu().numpy()
            else:
                label_masks = np.append(label_masks, l_mask.detach().cpu().numpy(), axis=0)
        ...
        results.update(compute_metrics(cap_preds,out_cap_labels,punc_preds,out_punc_labels,label_masks))
    ...
    def load_model(self,path=None):
        ...
        self.trainer_state.load_from_json(os.path.join(path, "trainer_state.json"))
...
#         self.lr_scheduler.load_state_dict(torch.load(os.path.join(path, "scheduler.pt")))
