...
from src.spelling_error.typographic import FAT_FINGER_ERROR, TELEX_ERROR, VNI_ERROR
from src.utils.constants import ALPHABET, ALPHABET_VN
...
def get_possible_word_from_fat_finger_error(word):
    ...
    possible_words = [
        word[:position_error] + char + word[position_error + 1 :]
...
def get_possible_word_from_edit_error(word):
    ...
    type_edit = random.choices(
        ["delete", "insert", "permute", "replace"], weights=(35, 17, 39, 39), k=1
    )[0]
    ...
    if type_edit == "delete":  # Delete Error
        ...
        possible_words = [
            word[:position_error] + random.choice(ALPHABET_VN) + word[position_error:]
        ]
    elif type_edit == "permute":  # Permute Error
        if position_error == 0:
            possible_words = [
                word[position_error + 1]
                + word[position_error]
                + word[position_error + 2 :]
            ]
